# Write-up giải mã Palimpsest trên CI-runner

**Sự kiện:** Security Bootcamp 2026  
**Máy bị ảnh hưởng:** `172.16.201.104`  
**Chứng cứ và kết quả xử lý:** ngày 11/09/2026

## 1. Kết quả và phạm vi

116 file được xác thực và ghi phục hồi; kế hoạch không có lỗi. Lượt chính thu được **177.124.458 byte plaintext**. Kết quả được đối chiếu giữa kế hoạch giải mã và log áp dụng; thông tin dịch vụ chỉ dùng để kiểm tra sau phục hồi.

## 2. Thu thập chứng cứ và tìm frame

### Hiện trạng và artifact

`ci-initial.txt` ghi root filesystem `ro,relatime`; BusyBox và Python còn dùng được. Nhóm ghi nhận boot, mount và service, lấy vật liệu dẫn xuất trên host rồi xác thực container trước khi chuẩn bị thay file.

Gói `ci-recovery-material.tar` có bốn thành viên: `frame.bin`, `host-identity.bin`, `manuscript-fingerprint.bin`, `palimpsest_recover.py`. Đây là gói vật liệu phục hồi, **không phải RAM dump, backup toàn máy hay archive plan.json**. Kế hoạch/log và bản sao file mã hóa được lưu riêng. Các giá trị vật liệu khóa không chép vào báo cáo.
...
Máy CI-runner dùng lại **frame**, kết hợp **identity riêng của máy** và fingerprint từ SVG. Kết quả `ci-key-test.txt` ghi xác thực thành công. Đối chiếu trực tiếp `ci-recovery-material.tar` xác nhận frame khớp `recovery-edge/frame-candidate.bin`, identity dài 141 byte và fingerprint dài 32 byte. Không suy ra khóa cuối cùng giống nhau giữa host hoặc giữa các file.
...
- `ci-recovery-plan-summary.json`: 116 file xác thực, 177.124.458 byte plaintext trong lượt chính.
- `ci-applied.txt`: 116 file đã ghi phục hồi trong lượt chính, có SHA256 archive backup.
- Không có lỗi trong kế hoạch chính sau khi có vật liệu đúng.

Sau thay file, Docker và act_runner hoạt động. Journal ghi `.runner` còn tồn tại nên bỏ qua đăng ký mới, sau đó runner khai báo thành công. Việc giữ bản đăng ký hiện có giúp tránh tái tạo dữ liệu ứng dụng không cần thiết.

## 8. Giới hạn và bài học

Chưa chạy pipeline đầy đủ hoặc reboot sau giải mã. Log runner chỉ xác nhận khả năng khởi động/kết nối ở thời điểm kiểm tra. Dấu magic biến mất hoặc service khởi động không thay thế kiểm tra AEAD. Bản mã và hash phải được giữ để đối chiếu lại các kết luận.

## 9. Hồ sơ đối chiếu

- [Hiện trạng ban đầu](recovery-fleet/ci-initial.txt)
- [Thử vật liệu khóa](recovery-fleet/ci-key-test.txt)
- [Kế hoạch giải mã](recovery-fleet/ci-recovery-plan-summary.json)
- [Kết quả ghi phục hồi và hash backup](recovery-fleet/ci-applied.txt)
- [Kết quả riêng của máy](recovery-fleet/ci-runner-check.txt)
