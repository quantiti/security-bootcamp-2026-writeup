# Write-up giải mã Palimpsest trên app-runner

**Sự kiện:** Security Bootcamp 2026  
**Máy bị ảnh hưởng:** `172.16.203.110`  
**Chứng cứ và kết quả xử lý:** ngày 11/09/2026

## 1. Kết quả và phạm vi

105 file được xác thực và ghi phục hồi; kế hoạch không có lỗi. Lượt chính thu được **122.207.950 byte plaintext**. Kết quả được đối chiếu giữa kế hoạch giải mã và log áp dụng; thông tin dịch vụ chỉ dùng để kiểm tra sau phục hồi.

## 2. Thu thập chứng cứ và tìm frame

### Hiện trạng và artifact

`app-initial.txt` ghi root filesystem `ro,relatime`; BusyBox và Python còn dùng được. Nhóm ghi nhận boot, mount và service, lấy vật liệu dẫn xuất trên host rồi xác thực container trước khi chuẩn bị thay file.

Gói `app-recovery-material.tar` có bốn thành viên: `frame.bin`, `host-identity.bin`, `manuscript-fingerprint.bin`, `palimpsest_recover.py`. Đây là gói vật liệu phục hồi, **không phải RAM dump, backup toàn máy hay archive plan.json**. Kế hoạch/log và bản sao file mã hóa được lưu riêng. Các giá trị vật liệu khóa không chép vào báo cáo.
...
Máy app-runner dùng lại **frame**, kết hợp **identity riêng của máy** và fingerprint từ SVG. Kết quả `app-key-test.txt` ghi xác thực thành công. Đối chiếu trực tiếp `app-recovery-material.tar` xác nhận frame khớp `recovery-edge/frame-candidate.bin`, identity dài 141 byte và fingerprint dài 32 byte. Không suy ra khóa cuối cùng giống nhau giữa host hoặc giữa các file.
...
- `app-recovery-plan-summary.json`: 105 file xác thực, 122.207.950 byte plaintext trong lượt chính.
- `app-applied.txt`: 105 file đã ghi phục hồi trong lượt chính, có SHA256 archive backup.
- Không có lỗi trong kế hoạch chính sau khi có vật liệu đúng.

Journal sau phục hồi ghi watcher kết nối registry và cài package `@megacorp/utils` phiên bản 1.0.0. Bước khởi động ứng dụng có tạo dữ liệu runtime; dữ liệu đó không được tính là plaintext giải mã từ container. Kết quả giải mã dựa vào 105 bản ghi AEAD hợp lệ trong kế hoạch.

## 8. Giới hạn và bài học

Chưa reboot host sau giải mã. Việc watcher chạy không chứng minh mọi file ngoài phạm vi quét đã được khôi phục. Dấu magic biến mất hoặc service khởi động không thay thế kiểm tra AEAD. Bản mã và hash phải được giữ để đối chiếu lại các kết luận.

## 9. Hồ sơ đối chiếu

- [Hiện trạng ban đầu](recovery-fleet/app-initial.txt)
- [Thử vật liệu khóa](recovery-fleet/app-key-test.txt)
- [Kế hoạch giải mã](recovery-fleet/app-recovery-plan-summary.json)
- [Kết quả ghi phục hồi và hash backup](recovery-fleet/app-applied.txt)
- [Kết quả riêng của máy](recovery-fleet/app-app-final.txt)
